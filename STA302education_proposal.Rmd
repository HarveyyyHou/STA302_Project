...
#| warning: false
#| message: false
data <- read.csv("/Users/zhouxuanle/Desktop/STA302_Project/STA302_original_dataset.csv")
...
df_clean <- read.csv("/Users/zhouxuanle/Desktop/STA302_Project/cleaned_data_school.csv")
...
$R^2$, AIC, BIC, RSS, F and P value all improved from models that has the Squared Special Education predictor. We decide to drop school type, since all metrics does not differ by much, indicating that this predictor has minimal impact on the model, or no relationships. Finally, we can conclude the final model with 5 Predictors: School Language, Special Education Proportion, Special Education Squared, Low Income Proportion, Parents with minimal education proportion.
```{R}
...
#| fig-cap: Residual Plots
#| echo: false
p2 <- ggplot(data.frame(Fitted = model_drop_No_Add$fitted.values, Residuals = resid(model_drop_No_Add)), 
             aes(Fitted, Residuals)) +
  geom_point(color = "red", size = 1) +
...
p4 <- ggplot(data.frame(resid = resid(model_drop_No_Add)), aes(sample = resid)) +
  stat_qq() +
  stat_qq_line(color = "red") +
...
This study set out to explore how various school and family-related factors influence students’ success on the first attempt of the OSSLT in Ontario. By building a multiple linear regression model and refining it through transformations and diagnostic checks, we reached several important conclusions that deepen our understanding of student literacy outcomes.

## Key Findings

One of the most important findings is that **socioeconomic factors**—especially **family income** and **parental education level**—are strongly linked to OSSLT performance. Schools with more students from **low-income households** or with **parents lacking post-secondary education** tended to have **lower pass rates**, supporting earlier research like @zhang2020family. For every 1% increase in low-income students, the pass rate (transformed) decreased by 0.0116 units, while a 1% increase in students whose parents did not attend post-secondary education resulted in a 0.0310 unit drop.

The impact of **special education services** was also significant. By including both a linear and a squared term for this variable, the model revealed a **nonlinear, compounding negative effect**—performance declines became steeper as the percentage of students receiving special education increased. This aligns with @vaughn2014intensive findings that students in special education face greater academic challenges, even with support.

Surprisingly, **French-language schools outperformed English-language schools**, even though the OSSLT is conducted in English. This suggests that French schools may benefit from stronger instructional strategies, better resources, or more cohesive school environments, despite potential language barriers.

In contrast, **school type (Catholic vs. Public)** was not a significant predictor. Although previous research (e.g., @cheema2024difference) suggests that religious or structured schooling can enhance academic performance, our results indicate that other factors—like income and parental education—have a much stronger influence on OSSLT outcomes.
...
To address these limitations and build on our findings, future studies could:
...
- **Expand the Set of Predictors with Caution:** Including more variables, such as **teacher experience**, **school funding**, or **student engagement**, may improve model accuracy. However, to manage the risk of overfitting, regularization or **variable selection** should be applied alongside.
- **Validate with a Test Set:** To assess how well the model predicts new data, researchers should split the data into a **training set** and a **test set**, using the test set strictly for model evaluation. This simulates real-world prediction scenarios.
